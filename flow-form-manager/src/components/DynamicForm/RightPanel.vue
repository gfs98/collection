    ...
    <div class="field-box">
      ...
      <el-scrollbar class="right-scrollbar">
        ...
        <el-form v-show="currentTab==='field' && showField" size="small" label-width="90px">
          ...
          <el-form-item v-if="activeData['stroke-width'] !== undefined" label="进度条宽度">
            <el-input-number v-model.number="activeData['stroke-width']" />
          </el-form-item>
          <el-form-item v-if="activeData.percentage !== undefined" label="进度条进度">
            <el-input-number v-model.number="activeData.percentage" :min="0" :max="100" />
          </el-form-item>
          <el-form-item v-if="activeData.color !== undefined" label="进度条颜色">
            <el-color-picker v-for="(item,index) in activeData.color" :key="index" v-model="item.color"></el-color-picker>
            <el-divider>配置颜色</el-divider>
            <draggable
              ...
              handle=".option-drag"
            >
              <div v-for="(item, index) in activeData.color" :key="index" class="select-item">
                <!-- <div class="select-line-icon option-drag">
                  <i class="el-icon-s-operation" />
                </div> -->
                <el-color-picker :key="index" v-model="item.color"></el-color-picker>
                <el-input-number v-model.number="item.percentage" :min="0" :max="100" />
                <div
                  class="close-btn select-line-icon"
                  @click="activeData.color.splice(index, 1)"
                >
          ...
          <template v-if="['el-transfer'].indexOf(activeData.tag) > -1">
            <el-divider>选项</el-divider>
            <el-form-item label="数据类型">
              <el-radio-group v-model="activeData.dataType" size="small">
                <el-radio-button label="dynamic">动态数据</el-radio-button>
            ...
            <template v-if="activeData.dataType === 'static'">
              <draggable
                ...
                handle=".option-drag"
              >
                <div v-for="(item, index) in activeData.data" :key="index" class="select-item">
                  <div class="select-line-icon option-drag">
                    <i class="el-icon-s-operation" />
                  </div>
                  <!-- <el-input v-model="item.label" placeholder="选项名" size="small" /> -->
                  ...
                  <div
                    class="close-btn select-line-icon"
                    @click="activeData.data.splice(index, 1)"
                  >
                    <i class="el-icon-remove-outline" />
                  </div>
                </div>
...
import {mergeNumberOfExps, validExp} from '@/assets/utils/index.js'
...
export default {
  ...
  methods: {
    ...
    addColor() {
      this.activeData.color.push({
        color: "",
        percentage: ""
      });
    },
    addtransferItem() {
      // console.log(this.activeData.data)
      this.activeData.data.push({
        key: this.activeData.data.length?this.activeData.data.length+1:1,
        label: "",
        value: ""
      });
    },
