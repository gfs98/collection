...
export default {
	...
	methods: {
		...
		uploadFiles(files) {
			files = [].concat(files)
			this.uploadApiFiles(files, 5, res => {
				this.$emit('upload', {
					Files: res
				})
		 ...
		 * 批量上传
		 */
		uploadApiFiles(files, max = 5, onUploadProgress) {
			...
			return new Promise(resolve => {
				...
				function next() {
					...
					uni.uploadFile({
							...
							url: self.$api.apiBaseUrl + '/file/upload',
							name: 'files',
							header: {
								'Accept': 'application/json',
								'accessToken':self.$api.userToken.accessToken,
								'clientId':self.$api.clientId
							},
							success: async (uploadFileRes) => {
								let resData = JSON.parse(uploadFileRes.data)
								if(resData&&resData.code){
									self.$api.showError(resData)
								}else{
									resData[0].path = self.$api.apiBaseUrl+'/file/preview?id='+resData[0].id+'&accessToken='+self.$api.userToken.accessToken
									resData[0].name = fileItem.name
									resData[0].size = fileItem.size
									resData[0].uuid = Date.now()
									let filedata = await self.getFileData(resData[0] , self.fileMediatype)
									self.files.push(filedata)
									onUploadProgress && onUploadProgress(self.files)
									if (cur < len) {
										next()
									}
